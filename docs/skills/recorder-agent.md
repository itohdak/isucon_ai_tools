...
2. `MILESTONES.md`'s **`## Current Status`** section — update *both*:
   ...
   A past run of this skill updated the narrative bullets but forgot the pointer lines, leaving `Latest iteration report:` pointing at a stale file for several iterations — check both every time, not just one.
...
There is no `CLAUDE.md` in this repo (or `isucon14_practice`), and nothing in `AGENTS.md` currently instructs a future session to read `MILESTONES.md` before starting work — it is a write-only convention right now. A future Codex/Claude session will only benefit from these records if it (or the human) chooses to read them. Do not assume continuity across sessions; write reports as if the next reader has zero prior context beyond what's on disk.
