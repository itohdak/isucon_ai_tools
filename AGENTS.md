...
- **Third tuning session (2026-09-24, iterations 1-6): ~188k -> ~240k mean (4-host layout, app commit `6ca111f`).** Wins: session-cookie decode memoisation (`session_cache.go`, +4.5%), viewer-path round-trip cuts (livecomment/reaction GET+POST: immutable livestream/tag caches, one NG-word query with `COLLATE utf8mb4_general_ci`, no BEGIN/COMMIT around single statements, +14%), remaining viewer calls (`GET livestream/:id`, enter/exit, login, me, user, theme, +4%), nginx buffered access log + `keepalive_requests 100000` + `proxy_buffers 32 8k` (+2.7%), and a neutral `moderateHandler` correctness fix (single case-insensitive DELETE). **Key finding: never speed up `GET /api/livestream/search` (or the other list handlers) in isolation.** Batching its tag queries (iteration 1) and the earlier tag cache both cut the score ~15-20% because the bench then runs ~2.5x more `aggressive-streamer-moderate`/`viewer-spam` iterations and ~10% fewer `viewer` completions (`viewer` is what produces tips); reverting restored ~192-196k immediately. Conversely, speeding up the tip-producing viewer paths raised viewer completions from ~975 to ~1250. Read the scenario counts from the bench log (`viewer`, `aggressive-streamer-moderate`, `viewer-spam`) on every run; the helper is `scripts/isucon13_bench.sh` (prints them). Other durable facts: (1) pprotein's slowlog/httplog artifacts are only a 60 s window collected at `/api/initialize`, so they miss most of the load phase and its pprof artifact was empty for alternating runs; for full-run numbers stream `/var/log/nginx/access.log` (s1) and `/var/log/mysql/mysql-slow.log` (s2) to s4 and run `alp`/`slp` there. (2) `moderateHandler` had been comparing utf8mb4_bin columns (case-sensitive) while the original compared literals under the connection's utf8mb4_general_ci (case-insensitive): any rewrite of an NG-word LIKE must re-collate the column (`word COLLATE utf8mb4_general_ci`). (3) nginx 1.18's `keepalive_requests` default is 100 and default proxy buffers spool responses >36 KB to temp files (invisible at error log level). (4) Attempting to move nginx onto s3 (s1 83-90% busy vs s3 34-48%) was blocked by the auto-mode permission classifier before anything changed; it is an open, user-decision item (it would also need the zone address, bench `--webapp`, pprotein httplog target and `rotateLogs` changes). (5) At the end of the session s1 (isupipe ~105% + nginx ~50% of 200%) and s4 (bench ~1.7 cores) are both ~90% busy; s2 ~62%, s3 ~50%.
