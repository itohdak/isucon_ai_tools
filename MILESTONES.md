...
- Next candidates: a smarter icon-hash approach that only hashes when `If-None-Match` is present (avoiding iteration 7's regression); and, as the most significant remaining lever, an evidence-based app/DB host split onto the idle `s2` (same instance type/size, no scale-up) given the confirmed CPU saturation on `s1` (mysqld ~1 full core of 2) — would benefit from a fresh Resource Monitor dispatch against the current baseline first.
- Latest app optimization commit: `8dd8245` (isucon13_practice_3 repo on s1).
- Latest verified benchmark: avg 12628 (3 runs, all pass=true), commit `8dd8245`, 2026-09-23. Session-best single run: 13135.
- Latest iteration report: `reports/iterations/iteration-20260923-233000.md`.
