...
- Latest app optimization commit: `000d3fc match up to 3 rides per matching tick as a smaller step than the rejected 20x attempt`.
- Latest verified benchmark: `pass=true` across 5 consecutive runs (12927, 10595, 12519, 12684, 12667), average ≈12278 — roughly **2.2x** the pre-fix post-split average.
- Latest iteration report: `reports/iterations/iteration-20260923-004604.md`.
- The 20x matcher batch attempt (see below) was rejected for causing a catastrophic overload; retrying with a much smaller `maxMatchesPerCall = 3` succeeded cleanly across all 5 runs, confirming the fixed one-match-per-tick rate had been suppressing throughput far below what the DB-split infrastructure can sustain — a moderate step, not a 20x jump, was the right fix.
- Resource watch: post-3x-fix `pidstat` shows `s3`'s `mysqld` averaging ~169% CPU (up from ~141% right after the split), approaching its 2-vCPU ceiling as throughput has grown ~2.2x. If further throughput increases are attempted, check `s3` headroom first — it could become the next bottleneck.
