...
- Next candidates: `fillLivestreamResponse`'s per-tag N+1; a smarter icon-hash approach that avoids hashing on every unconditional request if revisited; and, if further query-level wins are exhausted, an evidence-based app/DB host split onto the idle `s2` (same instance type/size, no scale-up) given the confirmed CPU saturation on `s1`.
- Latest app optimization commit: `8aaa23d` (isucon13_practice_3 repo on s1) — content-equivalent to iteration 5's `efae626` (iterations 6 and 7 were both fully reverted).
- Latest verified benchmark: avg 12248.3 (iteration 5, 3 runs all pass=true, commit `efae626`); most recent single confirmation run at `8aaa23d` scored 11902, consistent with iteration 5's range, 2026-09-23.
- Latest iteration report: `reports/iterations/iteration-20260923-232000.md`.
